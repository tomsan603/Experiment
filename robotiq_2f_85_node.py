...
class Robotiq2F85ControllerNode(Node):
    def __init__(self):
        ...
        self.auto_close_low_service = self.create_service(Trigger, '/robotiq/auto_close/low', self.auto_close_low_force_callback)
        self.auto_close_medium_force_service = self.create_service(Trigger, '/robotiq/auto_close/low2', self.auto_close_medium_force_callback)
        self.auto_close_high_force_service = self.create_service(Trigger, '/robotiq/auto_close/low3', self.auto_close_high_force_callback)
        self.auto_open_low_service = self.create_service(Trigger, '/robotiq/auto_open/low', self.auto_open_low_force_callback)
        self.auto_open_medium_force_service = self.create_service(Trigger, '/robotiq/auto_open/low2', self.auto_open_medium_force_callback)
        self.auto_open_high_force_service = self.create_service(Trigger, '/robotiq/auto_open/low3', self.auto_open_high_force_callback)
    ...
    def auto_close_low_force_callback(self, _, response:Trigger.Response):
        # check if gripper is activated
        if self.activated is False:
            response.success = False
            response.message = 'Gripper is not activated! Command is skipped.'
        ...
        command.trigger_autorelease = 0
        command.autorelease_direction = 0
        # setting for low force (approx 1/3)
        command.position = 255  # 閉じる
        command.speed = 150     # 中程度の速度
        command.force = 85      # 低把持力 (255/3)
        # convert command to binary
        command_binary = self.generate_binary_command_from_2f_msg(command)
        # build the reponse
        response = Trigger.Response()
        response.success = self.communicator.send_command(command_binary)
        if response.success:
            response.message = 'LOW force auto-close request successful!'
            self.get_logger().info(response.message)
        else:
            response.message = 'LOW force auto-close request failed!'
            self.get_logger().error(response.message)
        return response

    def auto_close_medium_force_callback(self, _, response:Trigger.Response):
        # check if gripper is activated
        if self.activated is False:
            response.success = False
            response.message = 'Gripper is not activated! Command is skipped.'
        ...
        command.trigger_autorelease = 0
        command.autorelease_direction = 0
        # setting for medium force (approx 2/3)
        command.position = 255  # 閉じる
        command.speed = 200     # やや速い速度
        command.force = 170     # 中把持力 (255*2/3)
        # convert command to binary
        command_binary = self.generate_binary_command_from_2f_msg(command)
        # build the reponse
        response = Trigger.Response()
        response.success = self.communicator.send_command(command_binary)
        if response.success:
            response.message = 'MEDIUM force auto-close request successful!'
            self.get_logger().info(response.message)
        else:
            response.message = 'MEDIUM force auto-close request failed!'
            self.get_logger().error(response.message)
        return response

    def auto_close_high_force_callback(self, _, response:Trigger.Response):
        # check if gripper is activated
        if self.activated is False:
            response.success = False
            response.message = 'Gripper is not activated! Command is skipped.'
        ...
        command.trigger_autorelease = 0
        command.autorelease_direction = 0
        # setting for high force (max)
        command.position = 255  # 閉じる
        command.speed = 255     # 最高速
        command.force = 255     # 高把持力 (Max)
        # convert command to binary
        command_binary = self.generate_binary_command_from_2f_msg(command)
        # build the reponse
        response = Trigger.Response()
        response.success = self.communicator.send_command(command_binary)
        if response.success:
            response.message = 'HIGH force auto-close request successful!'
            self.get_logger().info(response.message)
        else:
            response.message = 'HIGH force auto-close request failed!'
            self.get_logger().error(response.message)
        return response

    def auto_open_low_force_callback(self, _, response:Trigger.Response):
        # check if gripper is activated
        if self.activated is False:
            response.success = False
            response.message = 'Gripper is not activated! Command is skipped.'
        ...
        response.success = self.communicator.send_command(command_binary)
        if response.success:
            response.message = 'LOW force auto-open request successful!'
            self.get_logger().info(response.message)
        else:
            response.message = 'LOW force auto-open request failed!'
            self.get_logger().error(response.message)
        return response

    def auto_open_medium_force_callback(self, _, response:Trigger.Response):
        # check if gripper is activated
        if self.activated is False:
            response.success = False
            response.message = 'Gripper is not activated! Command is skipped.'
        ...
        response.success = self.communicator.send_command(command_binary)
        if response.success:
            response.message = 'MEDIUM force auto-open request successful!'
            self.get_logger().info(response.message)
        else:
            response.message = 'MEDIUM force auto-open request failed!'
            self.get_logger().error(response.message)
        return response

    def auto_open_high_force_callback(self, _, response:Trigger.Response):
        # check if gripper is activated
        if self.activated is False:
            response.success = False
            response.message = 'Gripper is not activated! Command is skipped.'
        ...
        response.success = self.communicator.send_command(command_binary)
        if response.success:
            response.message = 'HIGH force auto-open request successful!'
            self.get_logger().info(response.message)
        else:
            response.message = 'HIGH force auto-open request failed!'
            self.get_logger().error(response.message)
        return response
